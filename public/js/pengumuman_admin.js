...
$('#btn-buat-pengumuman').click((e) => {
  $('#section-pengumuman').show()
})

$('#btn-send-pengumuman').click((e) => {
  const pengumuman = $('#input-pengumuman').val()
  const date = new Date()

  firebase.database().ref('pengumuman').child(date.getFullYear()).child(date.getMonth() + 1).child(date.getDate()).push().set(pengumuman)
  $('#input-pengumuman').val('')

  alert('Pengumuman berhasil dibuat')
  $('#section-pengumuman').hide()
})

$('#btn-cancel-pengumuman').click((e) => {
  $('#section-pengumuman').hide()
})
...
$('#input-pengumuman').on('keypress', function(e) {
  if(e.which == 13) {
    const pengumuman = $('#input-pengumuman').val()
    const date = new Date()
